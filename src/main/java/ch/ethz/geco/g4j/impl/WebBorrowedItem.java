...
public class WebBorrowedItem implements BorrowedItem {
    ...
    public Mono<Void> remove() {
        return ((DefaultGECoClient) client).REQUESTS.makeRequest(Requests.METHOD.DELETE, "/lan/user/" + userID + "/items/" + id, null, null).doOnSuccess(aVoid -> {
            GECo4J.LOGGER.debug(LogMarkers.API, "Removed item <{}> from user <{}>", id, userID);
        }).then();
    }
}
